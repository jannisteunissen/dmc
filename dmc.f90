...
contains
  ...
  subroutine walkers_initialize(cfg, walkers)
    ...
    walkers%n_spin_down = n_particles - walkers%n_spin_up
  ...
  subroutine walkers_update(walkers, dt, trial_energy, sum_w, n_eff)
    ...
    real(dp)                       :: sum_w2
    real(fp)                       :: rr(n_dim, n_particles)

    sqrt_dt = sqrt(real(dt, fp))
    sum_w  = 0.0_dp
    sum_w2 = 0.0_dp

    !$omp parallel do private(phi_avg, n, rr, idim, ix, exp_arg, old_region) &
    !$omp &reduction(+: sum_w, sum_w2)
    do i = 1, walkers%n
       phi_avg = walkers%phi(i)
       old_region = walkers%region(i)

       call get_normal_numbers(n_dim, n_particles, &
  ...
  subroutine systematic_resample(walkers, sum_w)
    ...
    associate (cdf => walkers%cdf, x_new => walkers%x_new, &
         phi_new => walkers%phi_new, region_new => walkers%region_new)
